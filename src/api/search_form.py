...
from src.api.fpbase_client import FPbaseAPIError, FPbaseAPI
...
def format_results_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_cols = ['Em_Max', 'Ex_Max', 'Cross_Section', 'QY', 'EC', 'pKa', 'Brightness']
    for col in numeric_cols:
        if col in df.columns:
            df[col] = df[col].round(2)
    
    # Sort by emission wavelength by default
    if 'Em_Max' in df.columns:
        df = df.sort_values('Em_Max', ascending=True)
...
def search_proteins(query: str, filters: Dict[str, Any], client: FPbaseAPI) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        with st.spinner("🔍 Searching FPbase database..."):
            ...
            complete_data = len(results_df.dropna(subset=["Em_Max"]))  # Only check for emission data
    ...
    if st.button("🔍 Search", use_container_width=True, type="primary"):
        ...
        if result["success"]:
            ...
            with tab2:
                ...
                with col2:
                    st.metric("With Emission Data", 
                             result["data"]["Em_Max"].notna().sum())
                    if "QY" in result["data"].columns:
                        st.metric("Average QY", 
                                f"{result['data']['QY'].mean():.2f}")
        else:
