"""
    ...
    Classes/Functions:
        - calculate_virtual_wavelength(lambda_a: float, lambda_b: float) -> float (line 30)
        - calculate_tissue_parameters(wavelengths: npt.NDArray[np.float64], g: float = None, a: float = None, water_content: float = None, b: float = None, depth: float = None, normalization_wavelength: float = None, lambda_a: Optional[float] = None, lambda_b: Optional[float] = None) -> Dict[str, Any] (line 37)
        - create_tissue_plot(wavelengths: npt.NDArray[np.float64], tissue_data: Dict[str, Any], normalization_wavelength: float = 1300, absorption_threshold: float = 50, wavelength_range: Optional[Tuple[float, float]] = None, depth: float = 1.0) -> go.Figure (line 129)
...
def create_tissue_plot(
    ...
    if "T" in tissue_data and len(tissue_data["T"]) > 0:
        norm_idx = np.abs(wavelengths - normalization_wavelength).argmin()
    ...
    if "Tw" in tissue_data and len(tissue_data["Tw"]) > 0:
