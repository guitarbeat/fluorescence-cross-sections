...
def create_tissue_plot(
    ...
    absorption_mask = (
        tissue_data["Tw"] * 100 >= absorption_threshold
    ...
    while i < len(wavelengths) - 1:
        ...
        while (
            i < len(wavelengths) - 1
            and (absorption[i + 1] >= absorption_threshold) == is_above_threshold
        ):
            i += 1
    ...
    fig.update_yaxes(
        title_text="Water Absorption (%)", secondary_y=True, range=[0, 100]
    )
