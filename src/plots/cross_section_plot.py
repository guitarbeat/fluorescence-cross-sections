...
from ..plots.plot_utils import add_lasers_to_plot, apply_common_styling
...
def marker_settings_ui() -> None:
    ...
    with st.expander("Reference Style Settings", expanded=False):
        ...
        if st.button("Reset to Defaults", use_container_width=True):
            ...
            st.session_state.marker_settings = {
                ref: (default_markers[i % len(default_markers)], default_colors[i % len(default_colors)])
        ...
        for i, (ref, (marker, color)) in enumerate(st.session_state.marker_settings.items()):
            with preview_cols[i]:
